...
if (System.getProperty(ACTIVE_PROFILES_PROPERTY_NAME) == PRODUCTION) {
    logger("io.fourfinanceit", INFO)
    logger("com.ofg", INFO)
    logger("org.springframework.cloud.sleuth.log", OFF)
    logger("org.springframework.cloud.sleuth.util", OFF)
    logger("com.ofg.infrastructure.tracing", ERROR)
} else {
    logger("io.fourfinanceit", DEBUG)
    logger("com.ofg", DEBUG)
    //logger("org.springframework.jms", DEBUG)
    //logger("org.springframework.core", DEBUG)
